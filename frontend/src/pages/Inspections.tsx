...
export const Inspections = () => {
  ...
  useEffect(() => {
    if (user?.inspections) {
      console.log(`🔍 [Frontend] Nhận được ${user.inspections.length} hợp đồng.`);
      const invalidDates = user.inspections.filter(
        i => !i.createdAt || isNaN(new Date(i.createdAt).getTime())
      );
      if (invalidDates.length > 0) {
        console.warn("⚠️ Phát hiện hợp đồng bị lỗi ngày tháng:", invalidDates);
      }
    }
  }, [user?.inspections]);

  /**
   * Xử lý khi click "Xem lại phân tích" / click tên hợp đồng
   * Truyền thêm fileUrl + fileType + sessionId
   */
  ...
  return (
    <>
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              Xem lại và quản lý các hợp đồng đã được phân tích.
            </p>
          </div>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => handleActionClick(item as any)}>
                            ...
                            {item.score > -1 ? 'Xem lại phân tích' : 'Xem nội dung'}
                          </DropdownMenuItem>
                          <DropdownMenuItem
          ...
          {filteredInspections.length === 0 && (
            <div className="p-16 text-center text-slate-500">
              ...
              <p className="text-sm mt-1 mb-4">
                Các hợp đồng bạn phân tích sẽ được lưu trữ tại đây để tiện cho việc
                tra cứu.
              </p>
              <Button
                onClick={handleNewAnalysis}
            ...
            <div
              dangerouslySetInnerHTML={{
                __html:
                  ...
                  '<p class="text-gray-500 italic">Nội dung không khả dụng để xem trước.</p>',
              }}
            />
          </div>
      ...
      <Dialog open={!!deletingInspection} onOpenChange={() => setDeletingInspection(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Bạn có chắc chắn muốn xóa <strong>"{deletingInspection?.name}"</strong>? Hành
      ...
      {/* Alert khi đang có phân tích chưa lưu */}
      <AlertDialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Phân tích chưa được lưu</AlertDialogTitle>
            <AlertDialogDescription>
              Có một phân tích đang thực hiện chưa được lưu. Bạn có muốn hủy bỏ và bắt
              đầu phân tích mới không?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
